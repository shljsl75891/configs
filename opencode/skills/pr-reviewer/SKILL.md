...
**>25 files — multi-agent:** spawn parallel specialized @code-reviewer or @general subagents (one message, multiple Task calls), one per taxonomy tier the command defines focus items for (e.g. Security, Structural, Performance/Abstraction, Quality). Each subagent prompt must include:
