...
**>25 files — multi-agent:** spawn parallel `subagent` calls with `subagent_type: code-reviewer` (or `general-purpose`), one per taxonomy tier the command defines focus items for (e.g. Security, Structural, Performance/Abstraction, Quality). Each subagent prompt must include:
...
**Style**: write `Issue`/`Fix` text in Simplified Technical English (ASD-STE100) — short sentences, simple vocab, active voice, one instruction per sentence.
